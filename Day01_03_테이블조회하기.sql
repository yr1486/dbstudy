/*
    ...
    5. 형식( []는 생략 가능 )
        SELECT 다음 조회할 칼럼을 ,컴마로 구분해주면 됨. 몇개를 써도 상관없고 맨 마지막에 FROM 테이블 이름
        ...
        [GOUP BY 그룹화할칼럼 [HAVING 그룹조건식]]
        [ORDER BY 정렬할칼럼 정렬방식]
        
        [HAVING 그룹조건식] ==> 남자만 보겠다
        [ORDER BY 정렬할칼럼 정렬 방식] ==>( 조회. 순서. )정렬할 칼럼이 나오고 정렬 방식이 나온다, 높은거먼저 낮은거먼저 
...
/*
    ...
    3. 모든 세부 작업이 성공하면 COMMIT이라고 하고, 하나라도 실패하면 모든 세부 작업의 취소를 진행한다
        (All or Noting)
        ex) 휴면 계정 처리 같은거 할때, 
        어떤, 정보 테이블에서는 삭제하고, 휴면 테이블에는 넣어줘야하고 ==> 저기서 지운 정보를 다른 곳에다 넣을때 ==> 2개 이상의 작업을 모두 다 해야하고, 하나만 하면 소용없는 경우
...
-- 4. 테이블 별명 지정하기 ==> 테이블이름이 길때 사용하면 됨 줄임말임 ==> 별명은 문장안에서만 유효함. 진짜 이름을 바꿀수는 없음!! 그냥 긴 네임을 이 안에서 줄여서 쓴다는 거.
SELECT E.ENAME 
...
-- 2) 모든 칼럼 직접 작성하기
SELECT EMP, ENAME, JAB, MGR, HIREDATE, SAL, COMM, DEPTNO
  FROM EMP;
