...
1. 기본키(PK) : 고유한 식별자 키
    1) 개체무결성 
    => 모든 테이블이 기본키를 가져야 하고, 기본키로 선택된 열은 중복이 없는 상태여하며, 
        빈 값은 허용되지 않는다 , => 정확성 일관성 유효성
        널값이 올수 없다.
        테이블 내에 오직 하나의 값만 존재해야 한다.
    2) PK는 NOT NULL + UNIQUE 해야한다.
    
2. 외래키(FK)
    1. 참조무결성
    => 외래키의 값은 널이거나 참조릴레이션의 기본키 값과 동일해야함
    외래키의 속성은 참조할 수 없는 값을 지닐 수 없음 => 외래키 속성값이 상위 테이블의 인스턴스에
    반드시 존재하거나 널이여야 함. 
