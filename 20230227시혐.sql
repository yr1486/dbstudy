...
*/
SELECT
       TO_CHAR(BIRTHDATE, 'YYYY-MM-DD') AS 생일
  FROM STUDENT;
...
SELECT
       MONTHS_BETWEEN(SYSDATE, TO_DATE(HIREDATE, 'YY-MM-DD')) AS 근무일
  FROM PROFESSOR;
...
*/
SELECT DNAME AS 학과명, COUNT(*) AS 학생수
...
SELECT GRADE, AVG(HEIGHT) AS 평균키, AVG(WEIGHT) AS 평균몸무게
  FROM STUDENT
 GROUP BY GRADE
 ORDER BY DESC;
...
SELECT GET_STUDENT(USERID) AS 학번
  FROM STUDENT
 WHERE USERID ='jun123'; 
