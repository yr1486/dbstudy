...
CREATE OR REPLACE TRIGGER TRIG2
    AFTER
    INSERT OR UPDATE OR DELETE -- 작성순서없음
    ON EMPLOYEES
    FOR EACH ROW
BEGIN
...
-- TRIG2 동작 확인
INSERT INTO EMPLOYEES(EMPLOYEE_ID, LAST_NAME, EMAIL, HIRE_DATE, JOB_ID) VALUES(99, 'A', 'A', SYSDATE, 'A'); --빈문자열은 컴파일오류가나넹..
...
/*
    ...
    작업 순서
    1. 사원 테이블의 구조를 복사하여 퇴사자(RETIRES) 테이블을 생성한다. 
    ...
    5. RETIRE_TRIG 트리거를 생성한다.
        ...
        3) 퇴사번호는 시퀀스, 퇴사일자는 SYSDATE 로 처리해서 삽입한다.
*/
...
DROP SEQUENCE RETIRE_SEQ NOCACHE;


CREATE SEQUENCE RETIRES_SEQ NOCACHE; 

    
CREATE OR REPLACE TRIGGER RETIRES_TRIG
    AFTER
    DELETE
    ON EMPLOYEES
    FOR EACH ROW
BEGIN 
    INSERT INTO RETIRES(RETIRE_ID, RETIREE_DATE, EMPLOYEE_ID, FIRST_NAME, LAST_NAME, EMAIL, PHONE_NUMBER,
    HIRE_DATE, JOB_ID, SALARY, COMMITION_PCT, MANAGER_ID, DEPARTMENT_ID
    VALUES(RETIRE_SEQ.NEXTVAL, SYSDATE, :OLD.RETIRE_ID, :OLD.RETIREE_DATE, :OLD.EMPLOYEE_ID, :OLD.FIRST_NAME, :OLD.LAST_NAME, :OLD.EMAIL, :OLD.PHONE_NUMBER,
    :OLD.HIRE_DATE, :OLD.JOB_ID, :OLD.SALARY, :OLD.COMMITION_PCT, :OLD.MANAGER_ID, :OLD.DEPARTMENT_ID
END;
